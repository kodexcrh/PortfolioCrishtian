...
const IllustratorIcon = () => (
  <svg viewBox="0 0 36 36" width="28" height="28">
    ...
    <text x="5" y="26" fontFamily="Georgia,serif" fontSize="20" fontWeight="700" fill="#330000">
      Ai
    </text>
  </svg>
);
...
function LogoChip({ logo, className, dark }) {
  ...
  return (
    <div
      className={`${className} ${styles.logoChip}`}
      title={logo.name}
      style={{
        left: `${x}%`,
        top: `${y}%`,
        "--chip-color": logo.color,
        background: dark ? `${logo.color}18` : `${logo.color}15`,
        border: `1.5px solid ${logo.color}55`,
        boxShadow: `0 4px 16px ${logo.color}33`,
      }}
    >
...
function OrbeHero({ dark }) {
  return (
    <div className={styles.orbeWrapper}>
      ...
      <div
        className={`orbe-ring-3 ${styles.orbeRing}`}
        style={{
          width: "100%",
          height: "100%",
          border: `1px dashed ${dark ? "#01D4E822" : "#01D4E844"}`,
        }}
      />
      <div
        className={`orbe-ring-3 ${styles.orbeRing}`}
        style={{
          width: "94%",
          height: "94%",
          border: `1px solid ${dark ? "#01D4E82a" : "#01D4E850"}`,
          animation: "orbeRotate 22s linear infinite",
        }}
      >
        {RING3_LOGOS.map((logo) => (
          <LogoChip key={logo.name} logo={logo} className="logo-chip" dark={dark} />
        ))}
      </div>
      <div
        className={`orbe-ring-2 ${styles.orbeRing}`}
        style={{
          width: "74%",
          height: "74%",
          border: `1px solid ${dark ? "#01D4E828" : "#01D4E855"}`,
        }}
      >
        {RING2_LOGOS.map((logo) => (
          <LogoChip key={logo.name} logo={logo} className="logo-chip-2" dark={dark} />
        ))}
      </div>
      <div
        className={`orbe-ring-1 ${styles.orbeRing}`}
        style={{
          width: "62%",
          height: "62%",
          border: `1.5px solid ${dark ? "#01D4E844" : "#01D4E866"}`,
        }}
      >
        {RING1_LOGOS.map((logo) => (
          <LogoChip key={logo.name} logo={logo} className="logo-chip" dark={dark} />
        ))}
      </div>
...
// ── Hook useTyping — recibe roles como parámetro ───────────────
function useTyping({
  roles = [],
  typeSpeed = 80,
  deleteSpeed = 40,
  ...
  }, [displayed, isDeleting, roleIndex, roles, typeSpeed, deleteSpeed, pauseAfterType, pauseAfterDelete]);

  return { displayed, color: roles[roleIndex]?.color ?? "var(--clr-accent)" };
}

// ── Animated Stats ─────────────────────────────────────────────
function AnimatedStats({ T, styles, t }) {
  ...
  return (
    <div className="stats" ref={ref}>
      <div>
        ...
        <div className={styles.statLabel} style={{ color: T.textFaint }}>
          {t("about.yearsLabel")}
        </div>
      </div>
      <div>
...
export default function Hero({ dark, T }) {
  ...
  // Roles con textos traducidos y colores fijos
  const roles = [
    { text: t("hero.roles")[0], color: "var(--clr-accent)" },
    { text: t("hero.roles")[1], color: "var(--clr-pink)"   },
    { text: t("hero.roles")[2], color: "var(--clr-cyan)"   },
    { text: t("hero.roles")[3], color: "var(--clr-gold)"   },
  ];

  const { displayed: typedText, color: typingColor } = useTyping({ roles });
    ...
    <section id="inicio" className={styles.section}>
      ...
      >
        ...
        <motion.div className={`hero-anim ${styles.textCol}`} variants={itemVariants}>
          ...
          <motion.span className="stag" variants={itemVariants}>
            {t("hero.available") || "Disponible para proyectos"}
          </motion.span>

          <motion.h1 className="htitle" variants={itemVariants}>
            {t("hero.greeting").split("Crishtian")[0]}
            <span className="glow-text">Crishtian</span>
          </motion.h1>
          ...
          <motion.div className={styles.typingRow} variants={itemVariants}>
            <span
              ...
              style={{ color: typingColor, transition: "color .3s ease" }}
            >
              {typedText}
            </span>
            <span
          ...
          <motion.div className="hbtns" variants={itemVariants}>
            <button className="btn-p" onClick={() => scrollToSection("Proyectos")}>
              {t("hero.cta1")} →
            </button>
            <button className="btn-o" onClick={() => scrollToSection("Contacto")}>
              {t("hero.cta2")}
            </button>
          </motion.div>
                ...
                <div key={s.name} className="social-wrap">
                  <a href={s.href} target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none" }}>
                    <button
                      ...
                      style={{
                        background: hovSocial === s.name ? s.bg : dark ? "#0f0f1e" : "#f0eaff",
                        border: hovSocial === s.name ? "1.5px solid transparent" : "1.5px solid #7C3AED22",
                        boxShadow: hovSocial === s.name
          ...
          <motion.div variants={itemVariants}>
            <AnimatedStats T={T} styles={styles} t={t} />
          </motion.div>
        </motion.div>
