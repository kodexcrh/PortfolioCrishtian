...
export default function Precios({ dark, T }) {
  ...
  return (
    <section id="precios" className="sec">
      <div className="c">
        <Reveal><span className="stag">// {t("pricing.tag")}</span></Reveal>
        <Reveal delay={80}>
          <h2 className="sh2">
            {t("pricing.title")}{" "}
            <span className="glow-text">{t("pricing.titleSpan")}</span>
          </h2>
        </Reveal>
        <Reveal delay={140}>
            ...
            <Reveal key={plan.title} delay={i * 120} style={{ height: "100%", display: "flex", flexDirection: "column" }}>
              <div className="card-hover-wrap" style={{ flex: 1 }}>
                <div className={styles.card} style={{ background: T.bgCard, border: `1px solid ${plan.accent}28` }}>
                  ...
                  {plan.badge && (
                    <span className={`badge-pulse ${styles.badge}`}>{plan.badge}</span>
                  )}
            ...
            <div className="card-hover-wrap" style={{ flex: 1 }}>
              <div className={styles.card} style={{ background: T.bgCard, border: `1px solid ${activePlan.color}28` }}>
                <div
                  ...
                  style={{ background: `linear-gradient(90deg,${activePlan.color},${activePlan.color}66,transparent)`, transition: "background .4s" }}
                />
                <div className={styles.icon}><FaBolt /></div>
                <div className={styles.label} style={{ color: activePlan.color, transition: "color .3s" }}>
                  {t("pricing.automation.title")}
                </div>
                  ...
                  {AUTO_PLANS.map((p) => (
                    <button
                      ...
                      style={{
                        ...
                        color:       selectedPlan === p.id ? "#fff" : T.textSub,
                        borderRight: p.id !== "advanced" ? "1px solid var(--clr-accent)22" : "none",
                      }}
                    >
                      {p.label}
                    </button>
                ...
                <button
                  ...
                  style={{
                    marginTop:  16,
                    width:      "100%",
                    background: `linear-gradient(135deg,${activePlan.color}cc,${activePlan.color})`,
                    boxShadow:  `0 4px 20px ${activePlan.color}44`,
                    transition: "background .4s, box-shadow .4s",
                  }}
                  onClick={() => scrollToSection("Contacto")}
                >
                  {t("pricing.hireAuto")} {activePlan.label} →
                </button>
              </div>
            </div>
          </Reveal>
        ...
        <Reveal delay={300}>
          <p className={styles.note} style={{ color: T.textFaint }}>
            {t("pricing.note")}
          </p>
        </Reveal>
      </div>
